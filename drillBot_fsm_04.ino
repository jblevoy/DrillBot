...
void loop() {
    ...
    if ((machine_running == 1) && (millis() - count_timer_03 > 20) && (green_LED_on == 0)) {
        //Serial.println(hopper_pause);
        //Serial.println(state);
        //Serial.println(millis() - stepper_timer_16);
        count_14++;
...
//########################################################################################### SWITCH ###########################################################################################
    switch (state) {
        ...
        case HOPPER_TO_HOLE1:                                                               // feed from hopper to first hole location
            ...
            if (millis() - stepper_timer_16 > 2000 && hopper_pause == 1){
              stepper_timer_16 = millis();
            }
            if (millis() - stepper_timer_16 > 1000 && hopper_pause == 1){
              hopper_pause = 0;
            }
            digitalWrite(8, HIGH);                                                          // this moves the feed away from home
            if (((micros() - stepper_timer_07) > hold_step) && (step_half_07 == 0) && (hopper_pause == 0)) {
                digitalWrite(9, HIGH);
                stepper_timer_07 = micros();
                step_half_07 = 1;
            }
            if (((micros() - stepper_timer_07) > hold_step) && (step_half_07 == 1) && (hopper_pause == 0)) {
                digitalWrite(9, LOW);
                stepper_timer_07 = micros();
                step_half_07 = 0;
                count_04 ++;
        ...
        case DRILL_DOWN:
            digitalWrite(6, HIGH); // this moves the drill head away from home
            if (((micros() - stepper_timer_08) > 3000) && (step_half_08 == 0)) {
                digitalWrite(7, HIGH);
                stepper_timer_08 = micros();
                step_half_08 = 1;
            }
            if (((micros() - stepper_timer_08) > 3000) && (step_half_08 == 1)) {
                digitalWrite(7, LOW);
                stepper_timer_08 = micros();
                step_half_08 = 0;
                count_05 ++;
        ...
        case EJECT_TO_HOPPER:                                                               // switch 17
            ...
            if ((micros() - stepper_timer_15 > hold_step) && (step_half_15 == 0)) {
                digitalWrite(9, HIGH);
                stepper_timer_15 = micros();
                step_half_15 = 1;
            }
            if ((micros() - stepper_timer_15 > hold_step) && (step_half_15 == 1)) {
                digitalWrite(9, LOW);
                stepper_timer_15 = micros();
                step_half_15 = 0;
                count_10 ++;
